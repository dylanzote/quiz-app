  ...
  return (
    <PageCenter light justifyCenter>
      <CenterCardContainer>
        ...
        <p className="text-center text-xl leading-8 font-medium text-secondary-text">
          Take the Click Style Profile™ assessment to understand your unique communication 
          approach and learn how to leverage your strengths in any situation.
        </p>
        <div className="mt-12 mb-14">
          {quizTopics.map(({ title, icon, disabled }) => (
            <div
              ...
              onClick={() => !disabled && selectQuizTopic(title)}
              className={cn(
                'card-hover flex items-center justify-center rounded-2xl p-8 transition-all duration-500 max-w-md mx-auto border-2',
                disabled
                  ? 'bg-disabled-card cursor-not-allowed border-gray-300'
                  : 'bg-select-topic-bg cursor-pointer border-theme-color/20 hover:border-theme-color',
                quizTopic === title
                  ? 'border-theme-color shadow-lg bg-selected-answer'
                  : 'border-theme-color/20'
              )}
            >
              {icon}
              <span className="ml-4 text-xl font-semibold">
